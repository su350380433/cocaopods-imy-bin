...
cocoapods-imy-bin功能点：
 	...
 	1. 组件二进制化，无入侵式支持组件二进制化，同时满足多套环境，多种策略下的需求，以加快Ci打包速度、研发编译速度问题。
 	2. 本地配置文件 - Podfile_local
 	3. 二进制源码调试，类似[美团 iOS 工程 zsource 命令背后的那些事儿](https://links.jianshu.com/go?to=https%3A%2F%2Ftech.meituan.com%2F2019%2F08%2F08%2Fthe-things-behind-the-ios-project-zsource-command.html)的效果`pod bin code`。
...
- 静态资源服务器（可参考 [binary-server](https://github.com/su350380433/binary-server)）
- 二进制私有源（保存组件二进制版本 podspec）

初始化插件：

``` shell
 $ pod bin init                                                                          [14:02:00]

======  dev 环境 ========

开始设置二进制化初始信息.
所有的信息都会保存在 /Users/ci/.cocoapods/bin_dev.yml 文件中.
...
这里初始化信息除了`configuration_env`外，与`cocoapods-bin`一致。
...
```shell
#dev 初始化插件配置 默认dev环境
pod bin init --bin-url=https://gitlab.meiyou.com/Youbaobao/cocoapods-imy-bin-config/raw/master/bin_dev.yml

#Debug_iPhoneos 初始化插件配置
pod bin init --bin-url=https://gitlab.meiyou.com/Youbaobao/cocoapods-imy-bin-config/raw/master/bin_debug_iphoneos.yml


#release_iPhoneos 初始化插件配置，release暂时未启用
pod bin init --bin-url=https://gitlab.meiyou.com/Youbaobao/cocoapods-imy-bin-config/raw/master/bin_release_iphoneos.yml
```
...
```ruby
pod bin code YYModel_Lib 
```

`YYModel_Lib`为需要源码调试的组件库名称。成功之后像平时一样单步调试，控制台打印变量。让我们同时拥有使用二进制的便利和源码调试的能力。
